...
lib/csv/*                       The ingest pipeline (parse, columns, normalize, dedup, storage, operator)
...
**Upload (Drive):** `drive-import` → `syncDriveAction` / `importFromDriveAction` → `downloadFile` → same `uploadOne` + `commitBatch` path, tagged with `driveFileId`. Already-imported files are skipped by `driveFileId`; legacy untagged matches are backfilled.

**Commit:** `commitBatch(batchId)` — atomic status claim → load staged preview → re-classify against current DB → upsert in 100-row transactions (nested write: channel + observation) → set `imported` → discard preview. On any error: set `failed`, store message in `notes`. See `docs/CSV-PIPELINE.md`.
...
4. **`app/actions/upload.ts`** — add it to `channelData` (and `observationData` if applicable) in `commitBatch`.
